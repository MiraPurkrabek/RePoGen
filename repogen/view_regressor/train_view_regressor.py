...
from model import (
    RegressionModel,
    SphericalDistanceLoss,
    MSELossWithRegularization,
    SphericalDotProductLoss,
)
...
def parse_args():
    ...
    parser.add_argument(
        "--cpu", action="store_true", default=False, help="Will force CPU computation"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        ...
        for batch_x, batch_y in dataloader:
            ...
            if args.distance.upper() == "EUCLIDEAN":
                test_distance = np.linalg.norm(
                    y_pred.cpu().numpy() - batch_y.cpu().numpy(), axis=1
                )
            elif args.distance.upper() == "SPHERICAL":
                test_distance = angular_distance(
                    y_pred.cpu().numpy(), batch_y.cpu().numpy()
                )
            ...
            test_pdf, test_theta, test_phi = plot_heatmap(
                y_pred.cpu().numpy(),
                args.spherical_output,
                return_img=True,
                return_angles=True,
            )
                    ...
                    in_img = (
                        cv2.imread(
                            os.path.join(
                                ...
                                "..",
                                "val2017",
                                "{:012d}.jpg".format(int(image_ids[idx])),
                            )
                        ),
                    )
...
def main(args):
    ...
    for epoch in tqdm(range(args.epochs), ascii=True):
        ...
        if args.verbose and (epoch) % args.test_interval == 0:
            ...
            print(
                "Epoch [{:5d}/100]\tLoss: {:7.4f}\tElapsed: {:5.2f} s\tRemaining: {:5.2f} s".format(
                    epoch + 1, loss.item(), elapsed_time, remaining_time
                )
            )
