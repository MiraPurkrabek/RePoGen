...
def plot_testing_data(y_test_pred, is_spherical=False):
    ...
    ax.scatter(
        ...
        y_test_pred[:, 1],
        y_test_pred[:, 2],
        label="Predicted positions (n={:d})".format(len(y_test_pred)),
    )
...
def plot_heatmap(pts, is_spherical=False, return_img=False, return_angles=False):
    ...
    if return_img:
        ...
        ax1.set_ylabel("phi")
        plt.suptitle(
            "Distribution of samples, average distance = {:.2f}".format(radius)
        )
        ...
        plt.savefig(
            os.path.join(
                "images", "heatmaps", "heatmap_distance_{:.1f}.png".format(radius)
            )
        )
        plt.show()
...
def plot_training_data(
    ...
    if test_positions.shape[1] == 2:
        ...
        a = (
            np.sin(dlat / 2.0) ** 2
            + np.cos(phi_pos) * np.cos(phi_y) * np.sin(dlon / 2.0) ** 2
        )
    ...
    ax2.plot(
        np.arange(len(sorted_test_dist)),
        sorted_test_dist,
        label="Distance of test samples",
    )
    ax2.legend()
    ax2.grid()
