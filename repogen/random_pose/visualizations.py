...
def draw_pose(img, kpts, joints_vis, draw_style="custom"):
    ...
    if draw_style.startswith("openpose"):
        ...
        joints_vis = joints_vis[
            [0, 0, 6, 8, 10, 5, 7, 9, 12, 14, 16, 11, 13, 15, 2, 1, 4, 3]
        ]
    ...
    for bi, bone in enumerate(skeleton):
        ...
        if draw_style.startswith("openpose"):
            if draw_style == "openpose_vis" and not (
                joints_vis[b[0]] and joints_vis[b[1]]
            ):
                continue
            ...
            img = cv2.line(img, start, end, thickness=1, color=(0, 0, 255))
...
def draw_points_on_sphere(pts, score=None, show_axes=True):
    ...
    if score is not None:
        ...
        ax.scatter(
            pts[:, 0], pts[:, 1], pts[:, 2], c=1 - score, marker="o", cmap="rainbow"
        )
    else:
