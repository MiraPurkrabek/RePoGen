...
def random_3d_position_polar(distance=2.0, view_preference=None):
    ...
    if view_preference is None:
        ...
        # beta_range = np.pi
        raise ValueError(
            "view_preference must be specified. Use 'random_point_on_sphere' instead."
        )
...
def random_point_in_sphere(distance_min=0.3, distance_max=15.0):
    ...
    pt = (np.random.uniform(size=3) * scale + distance_min) * np.array(
        [random_sgn(), random_sgn(), random_sgn()]
    )
