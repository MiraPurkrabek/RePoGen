...
class VertexEdgeLoss(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            osp.exists(gt_edge_path) or gt_edges is not None
        ), "gt_edges must not be None or gt_edge_path must exist"
        assert (
            osp.exists(est_edge_path) or est_edges is not None
    ...
    def compute_edges(self, points, connections):
        edge_points = torch.index_select(points, 1, connections.view(-1)).reshape(
            points.shape[0], -1, 2, 3
        )
