...
from .utils import Tensor, batch_rodrigues, apply_deformation_transfer
...
def build_model_forward_closure(
    ...
    if per_part:
        ...
        assert cond, (
            'When per-part is True, "part", "part_key", "jidx" must not be' " None."
        )
...
def build_edge_closure(
    ...
    model_forward = build_model_forward_closure(
        body_model, var_dict, per_part=per_part, part_key=part_key, jidx=jidx, part=part
    )
...
def get_variables(
    batch_size: int, body_model: nn.Module, dtype: torch.dtype = torch.float32
) -> Dict[str, Tensor]:
    var_dict = {}
    ...
    if (
        body_model.name() == "SMPL"
        or body_model.name() == "SMPL+H"
        or body_model.name() == "SMPL-X"
    ):
        var_dict.update(
            {
                ...
                "betas": torch.zeros(
                    [batch_size, body_model.num_betas], dtype=dtype, device=device
                ),
            }
        )
    ...
    if body_model.name() == "SMPL+H" or body_model.name() == "SMPL-X":
        var_dict.update(
            left_hand_pose=torch.zeros(
                [batch_size, body_model.NUM_HAND_JOINTS, 3], device=device, dtype=dtype
            ),
